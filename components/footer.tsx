...
export default function Footer() {
  return (
    <footer className="relative w-full border-t border-white/[0.08] bg-white/[0.02] backdrop-blur-sm pt-16 pb-8 text-white mt-auto">
      {/* Background radial gradients for texture */}
      <div
        aria-hidden
        className="absolute inset-0 isolate z-0 opacity-40 contain-strict pointer-events-none"
      >
        <div className="absolute top-0 left-0 h-[320px] w-[140px] -translate-y-[87.5px] -rotate-45 rounded-full bg-[radial-gradient(68.54%_68.72%_at_55.02%_31.46%,rgba(255,255,255,0.06)_0,rgba(255,255,255,0.02)_50%,rgba(255,255,255,0.01)_80%)]" />
        <div className="absolute top-0 left-0 h-[320px] w-[60px] -rotate-45 rounded-full bg-[radial-gradient(50%_50%_at_50%_50%,rgba(255,255,255,0.04)_0,rgba(255,255,255,0.01)_80%,transparent_100%)] [translate:5%_-50%]" />
        <div className="absolute top-0 left-0 h-[320px] w-[60px] -translate-y-[87.5px] -rotate-45 rounded-full bg-[radial-gradient(50%_50%_at_50%_50%,rgba(255,255,255,0.04)_0,rgba(255,255,255,0.01)_80%,transparent_100%)]" />
      </div>

      <div className="relative mx-auto flex size-full max-w-7xl flex-col justify-between gap-5 px-6">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-4">
          <div className="w-full space-y-4">
            ...
            <p className="mt-8 text-white/50 text-sm md:mt-6 leading-relaxed">
              Beautifully crafted React components with physics-based motion.
            </p>
            <div className="flex gap-2 pt-2">
              {socialLinks.map((link, index) => (
                <a
                  ...
                  className="w-9 h-9 border border-white/10 rounded-lg flex items-center justify-center text-white/50 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
                >
                  {link.icon}
                </a>
              ))}
          ...
          {footerLinkGroups.map((group) => (
            <div className="w-full" key={group.label}>
              <div className="mb-10 md:mb-0">
                <h3 className="text-[13px] font-semibold tracking-wider text-white/90 uppercase mb-4">
                  {group.label}
                </h3>
                <ul className="space-y-3 text-white/50 text-[13px]">
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-white/[0.06] pt-8 mt-12 text-white/40 text-[13px] sm:flex-row">
          <p>
            &copy; {new Date().getFullYear()} Satori UI, All rights reserved.
          </p>
...
const footerLinkGroups: FooterLinkGroup[] = [
  {
    label: "Product",
    links: [
      { title: "Components", href: "/components/dotted-modern" },
