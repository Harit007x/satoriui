...
const ProButton = ({
  ...
  return (
    ...
    >
      <Box pixelColor={pixelColor} />
      <span className="font-medium text-white">{text}</span>
    </button>
  );
};

const Box = ({ pixelColor }: BoxProps) => {
  ...
  return (
    <div
      className={cn(
        "absolute inset-y-0 left-1 my-auto size-9 rounded-[4px] flex flex-col justify-center items-center gap-px transition-all duration-400 ease-out shadow-sm",
        colorMap[pixelColor],
      )}
    >
      {[0, 1, 2, 3, 4].map((row) => (
...
const Bubble = ({ highlight = false }: { highlight?: boolean }) => {
  return (
    <span
      className={cn(
